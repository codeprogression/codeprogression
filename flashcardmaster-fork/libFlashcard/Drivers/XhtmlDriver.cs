 ...
 *  Original FileName :  XhtmlDriver.cs
...
{
    /// <summary>
    /// Writes CardDeck to XHTML. Conversion to XHTML is driven by an XSLT.
    /// </summary>
    public class XhtmlDriver: XmlDriver
    {
        /*
         * The XML generated by the XmlDriver is altered by the means of 
         * 'MarkupStyles' and "UseRgbColor', which when set to true, uses 
         * WikiTextToXml() method to convert wikimarkup to XML and ouputs 
         * color in rgb(r, g, b) format, respectively.
         */

        public static bool ConvertMarkup = true;

        public XhtmlDriver(string path)
            : base(path) {
            base.canDeSerialize = false;
            base.canFullDeserialize = false;

            if (ConvertMarkup) {
                base.StyleProcessing = XmlTextStyleProcessing.ConvertToXml;
            } else {
                base.StyleProcessing = XmlTextStyleProcessing.Remove;
            }
            base.UseRgbColor = true;
        }

        public override LibFlashcard.Model.CardDeck DeSerialize(Stream stream) {
            // This is not possible because the structure of XHTML file processed by
            // an unknown XSLT is not predictable!
            // Note: It might be possible to write an XSLT to process an XHTML file into 
            //		 a compatible XML file--I doubt it will be worth the time!
            throw new NotSupportedException("XHTML files cannot be deserialized.");
        }
        ...
        public override void Serialize(System.IO.Stream stream, LibFlashcard.Model.CardDeck deck) {
            ...
            using (Stream xmlStream = new MemoryStream()) {
                ...
                base.Serialize(xmlWriter, deck);
                xmlStream.Seek(0, SeekOrigin.Begin);

                using (Stream xslStream = new MemoryStream(UTF8Encoding.Default.GetBytes(GetStyleSheet()))) {
                    // Load XSL to memory
                    XPathDocument xmlDoc = new XPathDocument(xmlStream);
                    XPathDocument xslDoc = new XPathDocument(xslStream);

                    XslCompiledTransform xslt = new XslCompiledTransform();
                    xslt.Load(xslDoc.CreateNavigator());

                    // Write transformed output
                    XmlTextWriter xmlResultWriter = new XmlTextWriter(stream, Encoding.UTF8);
                    xmlResultWriter.Formatting = Formatting.Indented;
                    xslt.Transform(xmlDoc.CreateNavigator(), xmlResultWriter);
